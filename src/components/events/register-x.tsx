...
import styl from './events.module.scss';


export default function NewEventModalX({eventId}: {eventId: string}) {
    ...
    return (
        <>
            <motion.button
                ...
                whileTap={{ scale: 1.05 }}
                className="openButtonnn rounded-md mb-6 text-sm font-bold uppercase shadow-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600"
            ...
            <AnimatePresence>
                {isOpen ? <Dialog eventId={eventId} close={() => setIsOpen(false)} /> : null}
            </AnimatePresence>
            <StyleSheet />
        </>
    )
...
function Dialog({ close, eventId }: { close: () => void, eventId: string }) {
  ...
  const registerEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('form state:', name, email, eventId );
    ...
    return (
        <>
            ...
            <motion.dialog
                ...
                ref={reff}
                className="modalll fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                open={false}
                onCancel={(event) => {
            ...
            >
                {!transit ? (
                    <form onSubmit={registerEvent} className="">
                    <Heading className="titlee capitalize opacity-80">Enter your details</Heading>
    ...
    <g id="Outline">
      <g>
        ...
        <circle cx="7.99995" cy="32" r="1"/>
                    ...
                    <p>registration successful. Check email.</p>
                </div>)}
                <button
                    className="closeButtonnn"
                    aria-label="Close"
                    onClick={close}
                >
                    <CloseIcon />
...
function StyleSheet() {
    ...
    &:hover, &:focus {
      ...
      color: #F5F5F5;
        ...
        .controlsss button.cancel {
            background-color: var(--divider);
            color: #f5f5f5;
        }

        .controlsss button.save {
            width: max-content;
            display: flex;
            justify-content: center;
            align-items: center;

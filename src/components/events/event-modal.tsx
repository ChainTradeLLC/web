...
import { Input, Textarea, Button, Select, SelectItem, useDisclosure, } from '@nextui-org/react';
...
function Dialog({ close }: { close: () => void }) {
  ...
  const handleEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('form state:', title, description, image, date, location, speakerIds);
    setBadgeState('processing');

    const res = await fetch('/api/events', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, description, image, date, location, speakerIds }),
    });
    ...
    return (
        <>
            ...
            >
              ...
              <label htmlFor="speakers" className="block text-sm font-medium text-white">
                        Speakers
                      </label>
                      <select
                        id="speakers"

      ...
      >
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className={`${styles.event_header} mx-auto max-w-2xl lg:mx-0`}>
            ...
            >
              <span>Host:</span> {event.organizer?.name}
            </p>
            <h1
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-20 px-6 lg:px-8 xl:grid-cols-5 mt-50">
          <div className={`${styles.speaka} max-w-2xl xl:col-span-2`}>
            ...
            <p className="mt-6 text-lg/8 text-gray-600">
              Meet the experts who will be sharing their insights and knowledge
              at the event, bringing a wealth of experience and expertise to the
              event.
            </p>
          </div>
          <ul role="list" className="divide-y divide-gray-800 xl:col-span-3">
            {event.eventSpeakers.map((speak: any) => (
              <li
                key={speak.speaker.name}
                className="flex flex-col gap-10 py-12 first:pt-0 last:pb-0 sm:flex-row"
              >
                <img
                  alt=""
                  src={`${speak.speaker.image}`}
                  className="aspect-4/5 w-52 h-60 flex-none rounded-2xl object-cover"
                />
                <div className={"max-w-xl flex-auto"}>
                  <h3
                    className={`${styles.spek_title} text-lg/8 font-semibold tracking-tight text-gray-900`}
                  >
                    {speak.speaker.name}
                  </h3>
                  <p
                    className={`${styles.spek_role}  text-base/7 text-gray-600`}
                  >
                    {speak.speaker.job}
                  </p>
                  <p
                    className={`${styles.spek_bio} mt-6 text-base/7 text-gray-600`}
                  >
                    {speak.speaker.bio}
                  </p>
                </div>
